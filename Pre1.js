...
function tokenizeRaw(text) {
    ...
    for (const match of text.matchAll(regex)) {
		...
		console.log(token.value + String(pendingSpaces.length));
...
async function loadEpub(file) {
    ...
    for (const item of spineItems) {
		...
		const preTitleList = words.slice(0, 8);
		let titleList = [];
		for (const word of preTitleList) {
			titleList.push(word.value);
		}
		const backupTitle = titleList.join(" "); 
...
function showNextWord() {
    ...
    const wordBlock = book.chapters[currentChapter].words[currentIndex].value;
	const orpIndex = book.chapters[currentChapter].words[currentIndex].orp;
